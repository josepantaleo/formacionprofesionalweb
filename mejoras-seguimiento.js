...
function estadoExtension(d){
 ...
 if(edad>=aviso)return{codigo:"sin-conexion",clase:"warning",texto:"Sin conexiÃ³n",icono:"fa-signal",edad};
...
function antiguedad(s){if(!Number.isFinite(s))return"sin seÃ±al";if(s<60)return`hace ${Math.round(s)} s`;if(s<3600)return`hace ${Math.floor(s/60)} min`;return`hace ${Math.floor(s/3600)} h`}
...
async function guardarConfiguracion(){
 ...
 if(estado){estado.className=`tracking-status-badge ${ok?"ok":"danger"}`;estado.textContent=ok?"ConfiguraciÃ³n guardada":"No se pudo guardar"}
}
function asegurarPaneles(){
 ...
 if(!document.getElementById("filtroSeguimientoProfesor")){
  filtros.insertAdjacentHTML("beforeend",`<select id="filtroSeguimientoProfesor" aria-label="Filtrar por estado de extensiÃ³n"><option value="">Todas las extensiones</option><option value="activa">ExtensiÃ³n activa</option><option value="detenida">Seguimiento detenido</option><option value="sin-conexion">Sin conexiÃ³n</option><option value="interrumpida">Seguimiento interrumpido</option><option value="no-instalada">Sin extensiÃ³n informada</option></select><input id="filtroDominioSeguimientoProfesor" placeholder="Dominio actual" aria-label="Filtrar por dominio actual">`);
 ...
 if(!document.getElementById("configuracionSeguimientoProfesor")){
  ...
  p.innerHTML=`<div class="tracking-panel-heading"><h4><i class="fa-solid fa-shield-halved"></i> ConfiguraciÃ³n de seguimiento y bloqueo</h4><span id="estadoConfiguracionSeguimiento" class="tracking-status-badge">Clase actual</span></div><div class="tracking-config-grid">
  <div class="tracking-config-field"><label>Salidas antes del bloqueo<input id="configLimiteSalidas" type="number" min="1" max="50"></label></div><div class="tracking-config-field"><label>DuraciÃ³n mÃ­nima (segundos)<input id="configDuracionMinima" type="number" min="0" max="300"></label></div><div class="tracking-config-field"><label>Aviso sin conexiÃ³n (segundos)<input id="configAvisoOffline" type="number" min="15" max="600"></label></div><div class="tracking-config-field"><label>InterrupciÃ³n despuÃ©s de (segundos)<input id="configAlertaOffline" type="number" min="60" max="3600"></label></div><div class="tracking-config-field"><label>Conservar historial (dÃ­as)<input id="configRetencionDias" type="number" min="1" max="3650"></label></div><div class="tracking-config-field"><label><input id="configBloqueoAutomatico" type="checkbox"> Bloqueo automÃ¡tico</label></div>
  ...
  <div class="tracking-config-actions"><button class="btn btn-primary" id="guardarConfiguracionSeguimiento" type="button"><i class="fa-solid fa-floppy-disk"></i> Guardar configuraciÃ³n</button><button class="btn btn-secondary" id="restaurarConfiguracionSeguimiento" type="button"><i class="fa-solid fa-arrow-rotate-left"></i> Restaurar recomendados</button></div><p class="tracking-config-note"><i class="fa-solid fa-user-shield"></i> Las alertas requieren revisiÃ³n docente y nunca descuentan puntos automÃ¡ticamente. No se guarda la URL completa.</p>`;
...
function asegurarPopupLimite(){
 ...
 modal.querySelector("#guardarLimiteBloqueo").onclick=async()=>{
  ...
  if(!ok){error.textContent="No se pudo guardar. VerificÃ¡ la autorizaciÃ³n docente y las reglas de Firebase.";error.style.display="block";return}
...
function renderIncidencias(){
 ...
 panel.innerHTML=`<div class="tracking-panel-heading"><h4><i class="fa-solid fa-triangle-exclamation"></i> Incidencias de seguimiento</h4><span>${items.length} requieren revisiÃ³n</span></div><div class="tracking-incidents-summary">
 <div class="tracking-incident-stat"><small>Bloqueados</small><strong>${contar(x=>x.pantallaBloqueada===true)}</strong></div><div class="tracking-incident-stat"><small>Sin conexiÃ³n</small><strong>${contar(x=>["sin-conexion","interrumpida"].includes(estadoExtension(x).codigo))}</strong></div><div class="tracking-incident-stat"><small>Sin extensiÃ³n</small><strong>${contar(x=>estadoExtension(x).codigo==="no-instalada")}</strong></div><div class="tracking-incident-stat"><small>Dominios en alerta</small><strong>${contar(x=>claseDominio(x.seguimientoExtension?.dominioActual).clase==="danger")}</strong></div></div>
 <div class="tracking-incident-list">${items.slice(0,20).map(x=>{const n=x.estudiante.estudiante?.nombre||x.estudiante.nombreGoogle||x.estudiante.email||"Estudiante",grave=x.problemas.some(t=>/Interrumpida|No informada|bloqueada|Dominio/.test(t));return`<div class="tracking-incident-item ${grave?"danger":""}"><strong>${escapeHtml(n)}</strong><span>${x.problemas.map(escapeHtml).join(" Â· ")}</span></div>`}).join("")||'<p style="color:var(--text-muted);margin:0">No hay incidencias de seguimiento.</p>'}</div>`;
}
...
function agregarBotonesComunicacionFilas(){
 ...
 filas.forEach(fila=>{
  ...
  grupo.innerHTML=`<button type="button" class="btn btn-success btn-comunicacion-fila" title="Videollamada Jitsi con ${escapeHtml(email)}"><i class="fa-solid fa-video"></i><span>Jitsi Meet</span></button>`;
  grupo.querySelector(".btn-comunicacion-fila").onclick=()=>window.abrirJitsiConEstudiante?.(email);
...
function asegurarModalesRevision(){
 if(!document.getElementById("revisionSalidaPestanaModal")){
  ...
  modal.innerHTML=`<div class="modal-box tab-review-box" tabindex="-1"><div class="tracking-limit-header"><div class="tracking-limit-heading"><span class="tracking-limit-icon"><i class="fa-solid fa-clipboard-check"></i></span><div><h3>Revisar salida de pestaÃ±a</h3><p id="revisionSalidaResumen">ClasificÃ¡ la salida y agregÃ¡ una observaciÃ³n docente.</p></div></div><button class="btn btn-secondary" id="cerrarRevisionSalida" type="button"><i class="fa-solid fa-xmark"></i></button></div><div class="tab-review-form"><div class="tab-review-field"><label>Estado<select id="revisionSalidaEstado"><option value="pendiente">Pendiente</option><option value="revisada">Revisada</option><option value="justificada">Justificada</option><option value="no_justificada">No justificada</option><option value="permitida">Permitida</option><option value="advertencia">Advertencia</option></select></label></div><div class="tab-review-field"><label>Motivo<select id="revisionSalidaMotivo"><option value="">Sin especificar</option><option>InvestigaciÃ³n solicitada</option><option>Consulta de documentaciÃ³n</option><option>Acceso autorizado por docente</option><option>Error tÃ©cnico</option><option>NotificaciÃ³n del sistema</option><option>Cambio accidental</option><option>DistracciÃ³n</option><option>Uso no autorizado de IA</option><option>Red social o entretenimiento</option><option>Otro</option></select></label></div><div class="tab-review-field wide"><label>ObservaciÃ³n docente<textarea id="revisionSalidaObservacion" maxlength="1000" placeholder="Contexto, justificaciÃ³n o medida tomada"></textarea></label></div><div class="tab-review-field wide"><label>Etiquetas</label><div class="tab-review-tags">${["investigaciÃ³n","documentaciÃ³n","IA","red social","video","juego","tÃ©cnico","accidental","reincidente","autorizada"].map(x=>`<label><input type="checkbox" value="${x}"> ${x}</label>`).join("")}</div></div><div class="tab-review-field wide"><label><input id="revisionSalidaContabiliza" type="checkbox" checked style="width:auto"> Mantener esta salida en el conteo de revisiÃ³n</label></div></div><div id="revisionSalidaError" style="display:none;color:#fca5a5;font-size:.78rem;margin:.6rem 0"></div><div class="tracking-limit-actions"><button class="btn btn-secondary" id="cancelarRevisionSalida" type="button">Cancelar</button><button class="btn btn-primary" id="guardarRevisionSalida" type="button"><i class="fa-solid fa-floppy-disk"></i> Guardar revisiÃ³n</button></div></div>`;
 ...
 if(!document.getElementById("auditoriaSalidaPestanaModal")){
  ...
  modal.innerHTML=`<div class="modal-box history-modal-box"><div class="history-modal-header"><div><h3><i class="fa-solid fa-clock-rotate-left"></i> AuditorÃ­a de la salida</h3><p id="auditoriaSalidaSubtitulo">Historial inmutable de cambios docentes.</p></div><button class="btn btn-secondary" id="cerrarAuditoriaSalida" type="button"><i class="fa-solid fa-xmark"></i> Cerrar</button></div><div id="auditoriaSalidaContenido" class="tab-audit-list"></div></div>`;
...
function abrirRevisionSalida(grupoId){
 ...
 modal.querySelector("#revisionSalidaResumen").textContent=`${d.paginas} pÃ¡gina(s) Â· ${d.dominios.join(", ")||"sin dominio"} Â· ${Math.round(d.segundos)} segundos`;
...
async function guardarRevisionSalida(){
 ...
 if(["no_justificada","advertencia"].includes(revision.estado)&&!revision.observacion){error.textContent="AgregÃ¡ una observaciÃ³n para una salida no justificada o con advertencia.";error.style.display="block";return}
 ...
 if(!ok){error.textContent=`No se pudo guardar${window.ultimoErrorRevisionPestana?.code?` (${window.ultimoErrorRevisionPestana.code})`:""}. PublicÃ¡ las reglas actualizadas.`;error.style.display="block";return}
...
async function abrirAuditoriaSalida(grupoId){
 ...
 contenido.innerHTML=eventos.length?eventos.map(e=>`<article class="tab-audit-event"><strong>${escapeHtml(e.estadoAnterior||"pendiente")} â†’ ${escapeHtml(e.estadoNuevo||"")}</strong><div class="history-event-meta"><span><b>Fecha:</b> ${escapeHtml(new Date(ms(e.cambiadoEn)).toLocaleString("es-AR"))}</span><span><b>Docente:</b> ${escapeHtml(e.docente||"")}</span><span><b>Conteo:</b> ${e.contabilizabaAntes!==false?"incluida":"excluida"} â†’ ${e.contabilizaAhora!==false?"incluida":"excluida"}</span></div>${e.motivoNuevo?`<p><b>Motivo:</b> ${escapeHtml(e.motivoNuevo)}</p>`:""}${e.observacionNueva?`<p>${escapeHtml(e.observacionNueva)}</p>`:""}</article>`).join(""):'<p style="color:var(--text-muted)">TodavÃ­a no hay cambios de revisiÃ³n para esta salida.</p>';
}
function asegurarFiltrosHistorial(){
 ...
 filtros.insertAdjacentHTML("beforeend",`<label>Dominio<select id="historialPestanasDominio"><option value="">Todos</option></select></label><label>SecciÃ³n<select id="historialPestanasSeccion"><option value="">Todas</option></select></label><label>RevisiÃ³n<select id="historialPestanasEstado"><option value="">Todos los estados</option><option value="pendiente">Pendientes</option><option value="revisada">Revisadas</option><option value="justificada">Justificadas</option><option value="no_justificada">No justificadas</option><option value="permitida">Permitidas</option><option value="advertencia">Advertencias</option><option value="excluida">Excluidas del conteo</option></select></label><label>ClasificaciÃ³n<select id="historialPestanasClasificacion"><option value="">Todas</option><option value="allowed">Dominio permitido</option><option value="danger">Dominio en alerta</option><option value="warning">Sin clasificar</option></select></label><label>DuraciÃ³n mÃ­nima (s)<input id="historialPestanasDuracionMin" type="number" min="0" placeholder="0"></label><label>DuraciÃ³n mÃ¡xima (s)<input id="historialPestanasDuracionMax" type="number" min="0" placeholder="Sin lÃ­mite"></label><label>PÃ¡ginas por salida<select id="historialPestanasPaginas"><option value="">Cualquier cantidad</option><option value="1">1 pÃ¡gina</option><option value="2">2 o mÃ¡s</option><option value="5">5 o mÃ¡s</option><option value="10">10 o mÃ¡s</option></select></label><label>VersiÃ³n<select id="historialPestanasVersion"><option value="">Todas</option></select></label><label>Orden<select id="historialPestanasOrden"><option value="fecha-desc">MÃ¡s recientes</option><option value="fecha-asc">MÃ¡s antiguas</option><option value="duracion-desc">Mayor duraciÃ³n</option><option value="duracion-asc">Menor duraciÃ³n</option><option value="paginas-desc">MÃ¡s pÃ¡ginas</option><option value="dominio-asc">Dominio A-Z</option><option value="estado-asc">Estado de revisiÃ³n</option></select></label>`);
...
async function copiarResumenGrupo(grupo){
 const d=datosGrupo(grupo),lineas=[`Salida: ${new Date(d.inicio).toLocaleString("es-AR")}`,`DuraciÃ³n: ${d.segundos} segundos`,`SecciÃ³n: ${d.seccion||"Sin identificar"}`,`Estado: ${etiquetaEstado(d.revision.estado)[0]}`,`Conteo: ${d.revision.contabiliza===false?"Excluida":"Incluida"}`,...d.eventos.map((e,i)=>`${i+1}. ${e.dominioDestino||"desconocido"} - ${e.tituloDestino||"Sin tÃ­tulo"} (${e.duracionSegundos||0}s)`)];try{await navigator.clipboard.writeText(lineas.join("\n"))}catch{const area=document.createElement("textarea");area.value=lineas.join("\n");document.body.appendChild(area);area.select();document.execCommand("copy");area.remove()}
}
async function aplicarRevisionMasiva(cambios){
 if(!salidasSeleccionadas.size)return;const confirmacion=await mostrarConfirmacionDocente({tipo:"warning",icono:"fa-list-check",titulo:"Actualizar salidas seleccionadas",mensaje:`Se modificarÃ¡n ${salidasSeleccionadas.size} salidas.`,detalles:["El cambio quedarÃ¡ registrado con fecha y docente.","Los registros originales de navegaciÃ³n no se modifican."],confirmarTexto:"Aplicar cambios",confirmarIcono:"fa-check-double",confirmarClase:"btn-warning"});if(!confirmacion.confirmado)return;
 ...
 salidasSeleccionadas.clear();decorarSalidas();aplicarFiltrosHistorial();if(guardadas===0)alert("No se pudo actualizar ninguna salida. PublicÃ¡ las reglas actualizadas de Firestore.");
}
...
function renderAnalitica(grupos){
 ...
 const items=[["Incluidas",datos.length-excluidas,"fa-list-check"],["Excluidas",excluidas,"fa-eye-slash"],["Promedio fuera",`${promedio}s`,"fa-stopwatch"],["Mediana",`${mediana}s`,"fa-chart-simple"],["Salida mÃ¡s larga",`${maximo}s`,"fa-arrow-up"],["Con varias pÃ¡ginas",multipagina,"fa-route"],["Dominios en alerta",alertas,"fa-triangle-exclamation"],["Hora con mÃ¡s salidas",horaPico?`${String(horaPico[0]).padStart(2,"0")}:00`:"â€”","fa-clock"]];
...
function aplicarFiltrosHistorial(){
 ...
 const eventos=grupos.flatMap(g=>g.eventos),r=document.getElementById("resumenFiltroHistorialPestanas");if(r)r.textContent=`${eventos.length} visita(s) Â· ${grupos.length} salida(s)`;
