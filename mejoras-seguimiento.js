...
async function confirmarLecturaMensajeDocente(){
 if(!mensajeDocenteVisible)return;const overlay=document.getElementById("mensajeDocentePantalla"),mensajeConfirmado=mensajeDocenteVisible,compatible=mensajeConfirmado.canal==="documento-estudiante";marcarMensajeCompatibleLeido(mensajeConfirmado.id);mensajesDocentePendientes=mensajesDocentePendientes.filter(m=>m.id!==mensajeConfirmado.id);mensajeDocenteVisible=null;overlay.hidden=true;mostrarSiguienteMensajeDocente();const ok=compatible?await window.marcarMensajeDocenteCompatibleFirebase?.(mensajeConfirmado,"leido"):await window.marcarMensajeDocenteLeidoFirebase?.(mensajeConfirmado);if(!ok)console.warn("El mensaje se cerró, pero la confirmación quedó pendiente de sincronización.");
}
window.addEventListener("mensajes-docente-estudiante",evento=>{if(esSesionDocenteActual())return;const confirmados=new Set(idsMensajesCompatiblesLeidos()),recibidos=(Array.isArray(evento.detail)?evento.detail:[]).filter(m=>m.leido!==true&&!confirmados.has(String(m.id))),actualId=mensajeDocenteVisible?.id||"";mensajesDocentePendientes=recibidos.filter(m=>m.id!==actualId);if(mensajeDocenteVisible&&!confirmados.has(String(mensajeDocenteVisible.id)))mensajesDocentePendientes.unshift(mensajeDocenteVisible);mostrarSiguienteMensajeDocente()});
function recibirMensajeDocenteCompatible(mensaje={}){
 if(esSesionDocenteActual())return;const user=window.firebaseCurrentUser;if(!user||!mensaje.id||mensaje.estudianteUid!==user.uid||mensaje.leido===true||idsMensajesCompatiblesLeidos().includes(String(mensaje.id)))return;if(mensaje.recibido!==true)void window.marcarMensajeDocenteCompatibleFirebase?.(mensaje,"recibido");if(mensajeDocenteVisible?.id===mensaje.id||mensajesDocentePendientes.some(item=>item.id===mensaje.id))return;mensajesDocentePendientes.push({...mensaje,canal:"documento-estudiante"});mostrarSiguienteMensajeDocente();
}
